...
class AIClient(ABC):
    ...
    @abstractmethod
    async def generate_insight(self, prompt: str, system_prompt: Optional[str] = None) -> str:
        """AI 인사이트 생성"""
        pass
   ...
   - 조직과 개인에게 미칠 영향"""
        ...
        for section in sections:
            if '핵심 강점' in section or '**핵심 강점**' in section:
                lines = section.split('\n')[1:]
                strengths = [line.strip('- •*').strip() for line in lines if line.strip() and not line.startswith('#')][:3]
            elif '사각지대' in section or '**잠재적 사각지대**' in section:
                lines = section.split('\n')[1:]
                improvements = [line.strip('- •*').strip() for line in lines if line.strip() and not line.startswith('#')][:3]
            elif '개발 전략' in section or '**맞춤형 개발 전략**' in section:
                lines = section.split('\n')[1:]
                action_plans = [line.strip('- •*').strip() for line in lines if line.strip() and not line.startswith('#')][:5]
            elif '성장 시나리오' in section or '**변혁적 성장 시나리오**' in section:
                lines = section.split('\n')[1:]
                expected_outcomes = ' '.join([line.strip() for line in lines if line.strip() and not line.startswith('#')])
...
class AIClientFactory:
    ...
    @staticmethod
    def create_client(provider: Optional[AIProvider] = None) -> AIClient:
...
async def get_ai_client(provider: Optional[AIProvider] = None) -> AIClient:
    """AI 클라이언트 인스턴스 반환 (캐싱)"""
    if provider is None:
        provider = settings.DEFAULT_AI_PROVIDER
