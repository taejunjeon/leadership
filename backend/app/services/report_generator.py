...
class ReportGenerator:
    ...
    def _create_cover_page(self, user_data: dict, analysis_data: dict) -> list:
        ...
        elements.append(Paragraph("AI Leadership 4Dx", self.styles["CustomTitle"]))
        elements.append(Paragraph("리더십 분석 보고서", self.styles["CustomTitle"]))
        ...
        info_data = [
            ...
            [
                "분석일",
                datetime.fromisoformat(analysis_data["created_at"]).strftime(
                    "%Y년 %m월 %d일"
                ),
            ],
        ]
    ...
    def _create_executive_summary(self, analysis_data: dict) -> list:
        ...
        risk_color = {"low": "green", "medium": "orange", "high": "red"}.get(
            analysis_data["overall_risk_level"], "black"
        )
        ...
        scores_data = [
            ...
            [
                "People (사람)",
                f"{analysis_data['blake_mouton_people']:.1f}",
                self._get_score_evaluation(analysis_data["blake_mouton_people"]),
            ],
            [
                "Production (성과)",
                f"{analysis_data['blake_mouton_production']:.1f}",
                self._get_score_evaluation(analysis_data["blake_mouton_production"]),
            ],
            [
                "LMX (관계품질)",
                f"{analysis_data['lmx_score']:.1f}",
                self._get_score_evaluation(analysis_data["lmx_score"]),
            ],
        ]
    ...
    def _create_development_plan(self, analysis_data: dict) -> list:
        ...
        if plan:
            for i, item in enumerate(plan, 1):
                elements.append(
                    Paragraph(f"<b>{i}.</b> {item}", self.styles["BodyTextJustify"])
                )
                elements.append(Spacer(1, 0.2 * inch))
