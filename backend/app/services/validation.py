...
from app.core.supabase import supabase_service, SURVEY_RESPONSES_TABLE
...
class DataValidationService:
    ...
    def _validate_response_completeness(
        ...
        if missing_dimensions:
            result.add_error(
                f"응답 개수 불일치: {', '.join(missing_dimensions)}"
            )
            result.completeness_score = len(dimension_counts) / len(self.expected_counts)
            ...
            if len(values) > 1:
                ...
                if std_dev > 2.5:  # 높은 변동성
                    high_variance_dims.append(
                        f"{dim.value} (표준편차: {std_dev:.2f})"
                    )
        
        if high_variance_dims:
            result.add_warning(
                f"높은 응답 변동성: {', '.join(high_variance_dims)}"
            )
    ...
    async def _check_duplicate_response(
        ...
        try:
            # Supabase에서 중복 확인
            existing = supabase_service.table(SURVEY_RESPONSES_TABLE).select(
                "id, created_at, raw_responses"
            ).eq(
                "leader_id", 
