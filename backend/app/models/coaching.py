...
코칭 모델
"""
from sqlalchemy import Column, String, Text, Boolean, JSON, ForeignKey, Integer, Float, DateTime
...
class CoachingCard(Base, TimestampMixin):
    ...
    response_id = Column(UUID(as_uuid=True), ForeignKey("grid3.survey_responses.id"), nullable=False)
    leader_id = Column(UUID(as_uuid=True), ForeignKey("grid3.leaders.id"), nullable=False)
    
    # 코칭 내용
    card_type = Column(String)  # "listening_challenge", "confrontation_script", "trust_routine"
