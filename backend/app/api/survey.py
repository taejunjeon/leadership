...
from ..schemas.survey import SurveyResponse, SurveyStats, SurveySubmission
...
async def submit_survey(
    ...
    ai_provider: str | None = Query(
        None, description="AI provider to use for analysis (openai or anthropic)"
    ),
):
    """설문 응답 제출"""
    try:
        ...
        if not user_id:
            # 이메일로 사용자 검색
            user_result = (
                db.table("users").select("*").eq("email", submission.email).execute()
            )
...
async def get_user_responses(user_id: str):
    ...
    try:
        db = get_service_supabase()
        result = (
            db.table("survey_responses").select("*").eq("user_id", user_id).execute()
        )
...
async def get_survey_stats():
    ...
    try:
        ...
        # 전체 응답 수
        total_result = (
            db.table("survey_responses").select("id", count="exact").execute()
        )
        total_responses = total_result.count or 0

        # 평균 완료 시간
        time_result = (
            db.table("survey_responses").select("completion_time_seconds").execute()
        )
        if time_result.data:
            times = [
                r["completion_time_seconds"]
                for r in time_result.data
                if r["completion_time_seconds"]
            ]
