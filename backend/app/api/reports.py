...
async def generate_pdf_report(user_id: str, analysis_id: str | None = None):
    """PDF 보고서 생성"""
    try:
...
async def get_report_summary(user_id: str):
    ...
    try:
        ...
        # 트렌드 계산
        trend = {
            "people": _calculate_trend(
                [h["blake_mouton_people"] for h in history_result.data]
            ),
            "production": _calculate_trend(
                [h["blake_mouton_production"] for h in history_result.data]
            ),
        ...
        for style, count in style_distribution.items():
            style_percentile[style] = (
                (count / total_peers * 100) if total_peers > 0 else 0
            )
...
async def get_team_report(organization: str, department: str | None = None):
    """팀/조직 보고서"""
    try:
        db = get_service_supabase()
...
def _generate_team_recommendations(
    styles: dict, risks: dict, avg_scores: dict
) -> list[str]:
    """팀 개선 제안 생성"""
    recommendations = []
