...
async def get_analysis_history(user_id: str, limit: int = Query(10, ge=1, le=100)):
    """사용자의 분석 이력 조회"""
    try:
        db = get_service_supabase()
...
async def get_quick_analysis(user_id: str):
    ...
    try:
        ...
        # 실행 항목 3개
        action_items = (
            full_analysis.improvement_areas[:3]
        ...
        viz_data = {
            ...
            "candor": (
                full_analysis.dimensions.care + full_analysis.dimensions.challenge
            )
            / 2,
