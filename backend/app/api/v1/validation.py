...
from app.utils.anomaly import anomaly_detector, AnomalyScore
...
async def deep_analysis(
    response: SurveyResponseCreate,
    initial_result: DataValidationResult,
    user_id: str
):
    """백그라운드 심층 분석"""
    # TODO: AI 기반 심층 분석 구현
    ...
    for anomaly in anomalies:
        if anomaly.dimension == "people_vs_production" and anomaly.severity == "high":
            recommendations.append(
                get_message("People과 Production 점수의 균형을 맞추는 것이 중요합니다", lang)
            )
        elif anomaly.dimension == "all_high":
            recommendations.append(
                get_message("모든 영역에서 높은 점수는 현실적이지 않을 수 있습니다. 솔직한 자기평가가 필요합니다", lang)
            )
        elif anomaly.dimension == "candor_lmx_conflict":
            recommendations.append(
                get_message("높은 솔직함과 낮은 관계 품질의 조합은 개선이 필요합니다", lang)
            )
