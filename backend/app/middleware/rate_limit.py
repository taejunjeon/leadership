...
class RateLimiter:
    ...
    def _check_memory(
        ...
        self.memory_store[key] = [
            ts for ts in self.memory_store[key] if ts > now - window
        ]
    ...
    async def check_llm_usage(
        ...
        req_allowed, req_info = await self.check_rate_limit(
            request_key, limits["requests"], limits["window"]
        )
    ...
    async def _update_token_usage(self, key: str, tokens: int, window: int):
        """토큰 사용량 업데이트"""
        if self.redis_client:
            try:
                pipe = self.redis_client.pipeline()
...
class LLMUsageMonitor:
    ...
    async def get_usage_stats(
        self, user_id: str | None = None, period: str = "day"
    ) -> dict[str, Any]:
        """사용량 통계 조회"""
        # 기간 설정
        if period == "day":
